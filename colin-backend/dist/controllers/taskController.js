 ...
 */
const canAccessCaseId = async (req, caseId) => {
    ...
    if (isAssociateLikeRole(role)) {
        ...
        if (!me)
            return false;
        const c = await caseModel_1.default.findById(caseId).select('assignedTo');
        if (!c)
            return false;
        ...
        const hasTask = await taskModel_1.default.exists({ caseId, assignee: me });
...
const getTasksForCase = async (req, res) => {
    try {
...
const addTaskToCase = async (req, res) => {
    try {
        let caseId = req.params.caseId;
        if (Array.isArray(caseId))
            caseId = caseId[0];
        ...
        if (assigneeValue) {
            const assigneeUser = await (0, notifyService_1.findUserByAssigneeString)(assigneeValue);
...
const getAllTasks = async (req, res) => {
    try {
        ...
        const tasks = await taskModel_1.default.find(filter).sort({ dueDate: 1, createdAt: -1 });
        res.json(tasks);
    }
    catch {
        res.status(500).json({ message: 'Failed to fetch tasks.' });
...
const updateTask = async (req, res) => {
    try {
        ...
        const updated = await taskModel_1.default.findByIdAndUpdate(taskId, req.body, { new: true });
        if (!updated)
            return res.status(404).json({ message: 'Task not found.' });
        const changes = [];
...
const toggleChecklistItem = async (req, res) => {
    try {
        ...
        const item = task.checklist?.find((i) => String(i._id) === String(itemId));
        if (!item)
...
const getTimeLogsForTask = async (req, res) => {
    try {
        ...
        const logs = await taskTimeLogModel_1.default.find({ taskId: new mongoose_1.default.Types.ObjectId(taskId) }).sort({ loggedAt: -1 });
...
const addTimeLogToTask = async (req, res) => {
    try {
        const { taskId } = req.params;
        ...
        const log = await taskTimeLogModel_1.default.create(payload);
        await (0, auditService_1.writeAudit)({
            caseId: String(task.caseId),
            ...withActor(req),
            action: 'TASK_UPDATED',
