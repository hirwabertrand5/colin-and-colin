...
const canAssociateLikeAccessCase = (req, foundCase) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    if (!isAssociateLike((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
        return false;
    ...
    if (assignedTo && assignedTo === me)
        return true;
    const hasTask = yield taskModel_1.default.exists({ caseId: foundCase._id, assignee: me });
    return Boolean(hasTask);
});
// ---------- Templates ----------
const listActiveTemplates = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const templates = yield workflowTemplateModel_1.default.find({ active: true })
...
const listAllTemplates = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
            return res.status(403).json({ message: 'Forbidden.' });
        const templates = yield workflowTemplateModel_1.default.find({}).sort({ updatedAt: -1 }).lean();
        res.json(templates);
    }
    catch (_b) {
        res.status(500).json({ message: 'Failed to load workflow templates.' });
...
const getTemplateById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const t = yield workflowTemplateModel_1.default.findById(templateId);
        if (!t)
...
const createTemplate = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
            return res.status(403).json({ message: 'Forbidden.' });
        const created = yield workflowTemplateModel_1.default.create(req.body);
        // NOTE: We avoid writing audit here because your audit log requires a caseId.
        res.status(201).json(created);
    }
    catch (e) {
        res.status(500).json({ message: (e === null || e === void 0 ? void 0 : e.message) || 'Failed to create template.' });
    }
});
...
const updateTemplate = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
            return res.status(403).json({ message: 'Forbidden.' });
        const { templateId } = req.params;
        const updated = yield workflowTemplateModel_1.default.findByIdAndUpdate(templateId, req.body, { new: true });
        if (!updated)
            return res.status(404).json({ message: 'Template not found.' });
        res.json(updated);
    }
...
const deleteTemplate = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
            return res.status(403).json({ message: 'Forbidden.' });
        const { templateId } = req.params;
        const deleted = yield workflowTemplateModel_1.default.findByIdAndDelete(templateId);
        if (!deleted)
            return res.status(404).json({ message: 'Template not found.' });
...
// ---------- Instances ----------
const getWorkflowForCase = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role)) {
            const allowed = yield canAssociateLikeAccessCase(req, c);
            if (!allowed)
                return res.status(403).json({ message: 'Forbidden.' });
        }
        const inst = yield workflowInstanceModel_1.default.findOne({ caseId: new mongoose_1.default.Types.ObjectId(caseId) }).lean();
        if (!inst)
...
const initWorkflowForCase = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
            return res.status(403).json({ message: 'Forbidden.' });
        ...
        if (!c)
            return res.status(404).json({ message: 'Case not found.' });
        const exists = yield workflowInstanceModel_1.default.findOne({ caseId: c._id });
        if (exists)
        ...
        const template = yield workflowTemplateModel_1.default.findById(tId).lean();
        if (!template)
            return res.status(404).json({ message: 'Template not found.' });
        ...
        const inst = yield workflowInstanceModel_1.default.create({
            ...
            status: 'Active',
            currentStepKey: (_b = steps[0]) === null || _b === void 0 ? void 0 : _b.stepKey,
            steps,
        });
        ...
        yield (0, auditService_1.writeAudit)(Object.assign(Object.assign({ caseId: String(c._id), actorName: actor.actorName }, (actor.actorUserId ? { actorUserId: actor.actorUserId } : {})), { action: 'WORKFLOW_INSTANCE_CREATED', message: 'Workflow initialized from template', detail: `${template.name} v${template.version}` }));
        res.status(201).json(inst);
    }
    catch (e) {
        res.status(500).json({ message: (e === null || e === void 0 ? void 0 : e.message) || 'Failed to initialize workflow.' });
    }
});
...
const attachOutputDocument = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        const inst = yield workflowInstanceModel_1.default.findOne({ caseId: c._id });
        if (!inst)
        ...
        const doc = yield documentModel_1.default.findById(documentId);
        if (!doc)
        ...
        yield inst.save();
        const actor = actorFromReq(req);
        yield (0, auditService_1.writeAudit)(Object.assign(Object.assign({ caseId: String(c._id), actorName: actor.actorName }, (actor.actorUserId ? { actorUserId: actor.actorUserId } : {})), { action: 'WORKFLOW_OUTPUT_UPLOADED', message: 'Attached deliverable to workflow output', detail: `${stepKey} • ${outputKey} • ${doc.name || 'Document'}` }));
        res.json(inst);
    }
    catch (_b) {
...
const completeStep = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        if (!isAdmin((_a = req.user) === null || _a === void 0 ? void 0 : _a.role))
            return res.status(403).json({ message: 'Forbidden.' });
        ...
        yield c.save();
        const actor = actorFromReq(req);
        yield (0, auditService_1.writeAudit)(Object.assign(Object.assign({ caseId: String(c._id), actorName: actor.actorName }, (actor.actorUserId ? { actorUserId: actor.actorUserId } : {})), { action: 'WORKFLOW_STEP_COMPLETED', message: 'Completed workflow step', detail: `${stepKey} • ${step.title}` }));
        res.json(inst);
    }
    catch (_b) {
