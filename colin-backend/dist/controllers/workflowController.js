...
const previousActiveStatus = (status) => {
...
const updateCaseWorkflowProgress = async (c, inst) => {
    ...
    const existingPlannedAmount = typeof c.workflowProgress?.plannedValue?.amount === 'number'
    ...
    const actions = (inst.steps || []).flatMap((step) => (Array.isArray(step.actions) ? step.actions : []));
    const checkedActions = actions.filter((action) => Boolean(action?.done)).length;
    const actionTotal = actions.length;
    const percent = actionTotal > 0 ? Math.round((checkedActions / actionTotal) * 100) : 0;
    const actionCompletedAmount = actionTotal > 0 ? Math.round((existingPlannedAmount * percent) / 100) : completedAmount;
    c.workflowProgress = {
...
const toggleStepAction = async (req, res) => {
    try {
        ...
        if (nextDone) {
            const orderedSteps = (inst.steps || []).slice().sort((a, b) => (a.order || 0) - (b.order || 0));
            const flatActions = orderedSteps.flatMap((orderedStep) => (orderedStep.actions || []).map((action, idx) => ({
                step: orderedStep,
                action,
                idx,
                key: orderedStep.stepKey,
            })));
            const currentFlatIndex = flatActions.findIndex((item) => item.key === stepKey && item.idx === actionIndex);
            const previousIncomplete = flatActions.slice(0, currentFlatIndex).find((item) => !item.action?.done);
        ...
        if (!nextDone) {
            const orderedSteps = (inst.steps || []).slice().sort((a, b) => (a.order || 0) - (b.order || 0));
            const stepIndex = orderedSteps.findIndex((orderedStep) => orderedStep.stepKey === stepKey);
